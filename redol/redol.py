...
class RedolClassifier:
    ...
    def __init__(self, n_estimators=100, method="regular", pil=0.75, bootstrap=1.0, nearest_neighbours=None, n_jobs=8, max_depth=None):
    ...
    def fit(self, x, y):
        ...
        if self.nearest_neighbours:
            nn = NearestNeighbors()
            nn.fit(x)
            self.nn_indixes = nn.kneighbors(X=x, n_neighbors=self.nearest_neighbours, return_distance=False)
            self.original_y = y
        ...
        with pymp.Parallel(self.n_jobs) as p:
            for n_classifier in p.range(0, self.n_estimators):
                ...
                if self.nearest_neighbours and self.method == "no_pil":
                    try:
                        modified_x, modified_y = self._change_class_nearest_neighbours_no_pil(_x, _y, indexes=ind)
                ...
                if self.nearest_neighbours:
                    try:
                        modified_x, modified_y = self._change_class_nearest_neighbours(_x, _y, indexes=ind)
                    except TypeError as e:
                        err_msg = f'Nearest neighbours parameter must be None or an integer within (1, N), N being X instances. It was set to {self.nearest_neighbours}'
                        raise RedolClassifierException(err_msg)
    ...
    def _change_class_nearest_neighbours(self, x, y, indexes):
        ...
        updated_data = self._nearest_neighbours(percentage, random_data, updated_data, y, indexes)

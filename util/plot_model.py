...
def plot_model(clf, X, y, title, number_of_classes=2, predict_proba=True):
    ...
    elif number_of_classes is 2:
        # Two classes
        plt.scatter(X[np.where(aux[:, 2] == 0)[0][:50], 0], X[np.where(aux[:, 2] == 0)[0][:50], 1], c='purple', s=20, edgecolor='k', alpha=0.6)
        plt.scatter(X[np.where(aux[:, 2] == 1)[0][:50], 0], X[np.where(aux[:, 2] == 1)[0][:50], 1], c='yellow', s=20, edgecolor='k', alpha=0.6)
